...
from verstackLight.tools import timer, Printer
from verstackLight.lgb_metrics import get_pruning_metric, get_optimization_metric_func, define_objective, classification_metrics, regression_metrics, get_eval_score, print_lower_greater_better, supported_metrics
from verstackLight.args_validators import *
from verstackLight.optuna_tools import Distribution, OPTUNA_DISTRIBUTIONS_MAP, SearchSpace
...
class LGBMTuner:
    ...
    def __init__(self, **kwargs):
        ...
        n_jobs : int
            number of parallel jobs to run. The default is None which is all cores available minus 2 for safeguard.
    ...
    def n_jobs(self, value):
        if not value is None:
            if not self._is_int(value) : raise TypeError('n_jobs must be an integer')
        self._n_jobs = value
    # -------------------------------------------------------------------------
